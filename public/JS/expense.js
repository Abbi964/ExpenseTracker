    ...
    if(amount.value==='' || category.value==='' || description.value===''){
        ...
        try{
            ...
            if(expenseOrIncome==='expense'){
                response = await axios.post(`http://13.50.241.251:3000/expense/addexpense`,expOrIncInfo);
                // making an list item
                li = makeLi(response.data, amount.value, category.value, description.value,'listItemExp');
            }
            else{
                response = await axios.post(`http://13.50.241.251:3000/income/addincome`,expOrIncInfo);
                // making an list item
...
async function activatePremium(e){
    ...
    let response = await axios.get(`http://13.50.241.251:3000/order/purchasePremium`,{headers:{'Authorization':token}})
    ...
    let options = {
        ...
        "handler": async function(respo){
            ...
            console.log(respo)
            await axios.post(`http://13.50.241.251:3000/order/updateTransectionStatus`,{
            ...
            let res = await axios.get(`http://13.50.241.251:3000/user/makePremiumInLocalStorage`,{
                headers:{'Authorization': token}
            })
    ...
    rzp1.on('payment.failed',(res)=>{
        ...
        axios.post(`http://13.50.241.251:3000/order/transectionFalied`,{order_id:options.order_id},{headers:{'Authorization': token}})
    })
}
    ...
    if(e.target.className==='delBtn'){
        ...
        if(e.target.parentElement.parentElement.className==='expenseList'){
            try{
                ...
                let token = localStorage.getItem('token')
                let response = await axios.delete(`http://13.50.241.251:3000/expense/delete/${li_id}`,{
                    headers:{'Authorization':token}
                })
                // deleting from DOM
        ...
        // if we clicked on del btn of income list
            ...
            try{
                ...
                let li_id = e.target.parentElement.id;
                let token = localStorage.getItem('token')
                let response = await axios.delete(`http://13.50.241.251:3000/income/delete/${li_id}`,{
                    headers:{'Authorization':token}
                })
                // deleting from DOM
                let li = e.target.parentElement
...
async function loadExpensesAndIncome(e){
    try{
        ...
        let token = localStorage.getItem('token')
        let response = await axios.get(`http://13.50.241.251:3000/expense/all_expenses?page=${page}`,{ 
            headers:{ 'Authorization': token },
        ...
        let incResponse = await axios.get(`http://13.50.241.251:3000/income/all_income?page=${page}`,{ 
            headers:{ 'Authorization': token },
            params:{noOfrows:localStorage.getItem('noOfRows')}
        })
...
async function getIncomeAndShow(page){
    try{
        let token = localStorage.getItem('token')
        let response = await axios.get(`http://13.50.241.251:3000/income/all_income?page=${page}`,{ 
            headers:{ 'Authorization': token },
            params:{noOfrows:localStorage.getItem('noOfRows')}
        })
...
function addToTotalExpense(amount){
    let token = localStorage.getItem('token')
    axios.post(`http://13.50.241.251:3000/user/addToTotalExpense`,{
        amount:amount,
        token:token,
    })
}
...
function addToTotalIncome(amount){
    let token = localStorage.getItem('token')
    axios.post(`http://13.50.241.251:3000/user/addToTotalIncome`,{
        amount:amount,
        token:token,
    })
}
...
async function checkingAndapplyingPremium(){
    let token = localStorage.getItem('token')
    let result = await axios.get(`http://13.50.241.251:3000/user/ispremium`,{headers:{
        'Authorization':token
    }})
...
async function showLeaderboard(e){
    ...
    let response = await axios.get(`http://13.50.241.251:3000/premium/getLeaderboard`)
...
AllExpBtn.addEventListener('click',(e)=>{
    window.location.href = `http://13.50.241.251:3000/premium/getAllExpensePage`
})
