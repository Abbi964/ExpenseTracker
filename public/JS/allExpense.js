...
async function loadTableAndFileUrls(e){
    try{
        ...
        let result = await axios.get(`http://16.16.149.37:3000/premium/getAllExpenses`,{ headers:{ 'Authorization': token }})
        let count = 0
        ...
        //----------------loading fileUrls--------------------------//
        let response = await axios.get(`http://16.16.149.37:3000/user/getFileUrls`,{headers:{'Authorization':token}})
...
async function download(e){
    try{
        let token = localStorage.getItem('token')
        let result = await axios.get(`http://16.16.149.37:3000/user/download`,{headers:{ 'Authorization': token }})
         //saving fileUrl in DB
         let response = await axios.post(`http://16.16.149.37:3000/user/saveFileUrl`,{fileUrl:result.data.fileUrl},{headers:{ 'Authorization': token }})
